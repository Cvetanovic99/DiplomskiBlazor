...
public class DeleteCompanyAsAnonymousCommand : IRequest<Unit>
{
    public int CompanyId { get; set; }
    public bool IsAdminDeleting { get; set; }
}

public class DeleteCompanyAsAnonymousCommandValidator : AbstractValidator<DeleteCompanyAsAnonymousCommand>
{
    public DeleteCompanyAsAnonymousCommandValidator()
    {
        RuleFor(x => x.CompanyId).GreaterThan(0).WithMessage("CompanyId je obavezan");
    }
}

public class DeleteCompanyAsAnonymousCommandHandler : IRequestHandler<DeleteCompanyAsAnonymousCommand, Unit>
{
    ...
    public DeleteCompanyAsAnonymousCommandHandler(
    ...
    public async Task<Unit> Handle(DeleteCompanyAsAnonymousCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!request.IsAdminDeleting)
        {
            if (company.OwnerId is not null)
                throw new AppException(
                    "Nemate dozvolu da obrisete ovu kompaniju, u medjuvremenu je dodeljena vlasniku");
        }
